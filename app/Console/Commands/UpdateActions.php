...
class UpdateActions extends Command
{
    ...
    public function allActions($id){
        ...
        $client = new \GuzzleHttp\Client();
        $url = 'https://api.medilink.healthatom.com/api/v1/profesionales/'.$id.'/citas?q={"fecha":{"gt":"2023-04-19"},"estado_cita":{"eq":"Atendido"}}&sort=fecha:desc';
        ...
        while(True){
            foreach($body->data as $j => $data){
                ...
                // }
                sleep(2);
                if($data->fecha < '2023-03-28' or $data->fecha > '2023-05-30'){
                    continue;
                }

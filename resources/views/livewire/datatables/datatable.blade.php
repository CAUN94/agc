        ...
        <div class="flex justify-between items-center mb-1">
            <div class="h-10 flex items-center mb-2">
                ...
                <div class="w-96 flex rounded-lg shadow-sm">
                    <div class="relative flex-grow focus-within:z-10">
                        ...
                        <input wire:model.debounce.500ms="search" class="w-full pl-10 py-3 text-sm leading-4 block rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 focus:outline-none" type="text" />
            ...
            <div class="flex flex-wrap items-center space-x-1">
                ...
                <div x-data="{ init() {
                    ...
                    <button wire:click="export" class="flex items-center space-x-2 px-3 border border-green-400 rounded-md bg-white text-green-500 text-xs leading-4 font-medium uppercase tracking-wider hover:bg-green-200 focus:outline-none"><span>{{ __('Descargar') }}</span>
                    ...
                    @forelse($this->results as $row)
                        <div class="table-row p-1 {{ $this->rowClasses($row, $loop) }}">
                            @foreach($this->columns as $column)
                                ...
                                @else
                                    <div class="table-cell px-6 py-2 whitespace-nowrap @if($column['align'] === 'right') text-right @elseif($column['align'] === 'center') text-center @else text-left @endif {{ $this->cellClasses($row, $column) }}">
                                        {!! $row->{$column['name']} !!}
                                    </div>
                                @endif
                            @endforeach
                ...
                <div class="p-2 sm:flex items-center justify-between">
                    ...
                    @if(count($this->results))
                        ...
                        <div class="flex justify-end text-gray-600">
                            {{__('Resultados')}} {{ $this->results->firstItem() }} - {{ $this->results->lastItem() }} {{__('of')}}
                            {{ $this->results->total() }}
                        </div>
                    @endif
                </div>
